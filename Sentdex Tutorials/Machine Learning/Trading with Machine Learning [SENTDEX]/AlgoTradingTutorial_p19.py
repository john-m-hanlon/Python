	...
	if patFound == 1:
		...
		if predictionAverage < 0:
			...
			if realMovement < patForRec[29]:
				accuracyArrary.append(100)
			else:
				accuracyArrary.append(0)
...
#this can be simpified to reduce processing
	patternAr = []
	performanceAr = []
	patForRec = []
	...
	#moveOn = input('Press ENTER to contine...')

	samps+= 1
	toWhat += 1
	accuracyAverage = (sum(accuracyArrary)/(len(accuracyArrary)+1))
	print('Backtested Accurarcy is',str(accuracyAverage)+'% after',samps,'samples.')
